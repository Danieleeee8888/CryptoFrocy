...
class CryptoApp:
    def __init__(self, root):
        ...
        self.indicators = {
            ...
            'EMA50 Volume': tk.BooleanVar()  # Nuova selezione per EMA50 dei volumi
        }
        ...
        # Etichetta per i semafori di controllo
        ...
        # Aggiungi i semafori di controllo raggruppati
        for group, indicators in traffic_light_groups.items():
            group_frame = tk.Frame(traffic_lights_frame)
            ...
            for key in indicators:
                ...
                tk.Label(frame, text=traffic_light_labels[key], font=("Helvetica", 10)).pack(side=tk.LEFT)
                self.traffic_lights[key] = {
                    'left': tk.Label(frame, text="●", fg="black", font=("Helvetica", 20)),
                    'right': tk.Label(frame, text="●", fg="black", font=("Helvetica", 20))
                }
                self.traffic_lights[key]['left'].pack(side=tk.LEFT, padx=5)
                self.traffic_lights[key]['right'].pack(side=tk.LEFT, padx=5)
    ...
    def get_columns(self):
        ...
        if self.indicators['EMA50 Volume'].get():  # Aggiungi la colonna per EMA50 dei volumi se selezionata
    ...
    def download_ohlcv(self):
        try:
            ...
            if self.indicators['EMA50 Volume'].get():  # Calcola la EMA50 dei volumi se selezionata
            ...
            if self.indicators['EMA50 Volume'].get():  # Aggiungi il valore della EMA50 dei volumi se selezionata
                values.append(round(latest_candle['EMA50_Volume'], 2))
            
            # Inserisci la riga nella tabella e imposta in grassetto
            row_id = self.tree.insert("", "end", values=values)
            self.tree.tag_configure('bold_font', font=('Helvetica', 10, 'bold'))
            self.tree.item(row_id, tags=('bold_font',))

            # Aggiorna i semafori di controllo
            conditions = check_conditions(df)
            self.update_traffic_lights(conditions)
    ...
    def update_traffic_lights(self, conditions):
        for key, status in conditions.items():
            if status is None:  # Condizione non verificabile per assenza di dati
                self.traffic_lights[key]['left'].config(fg='black')
                self.traffic_lights[key]['right'].config(fg='black')
            elif status:  # Condizione verificata
                self.traffic_lights[key]['left'].config(fg='black')
                self.traffic_lights[key]['right'].config(fg='green')
            else:  # Condizione non verificata
                self.traffic_lights[key]['left'].config(fg='red')
                self.traffic_lights[key]['right'].config(fg='black')
